...
#define AUTOSHEDDING_H_INCLUDED
   ...
   ///Function to make a loadshedding schedule and print it in a file
    void auto_schedule_make_using_default_time(Time time_a_day,char file_name[32])
    {
    ///define variables
    ...
    ///Initialization
        ...
        grps[1].day[1].morning[0]=use_default_ls_time('m');
        grps[1].day[1].night[0]=use_default_ls_time('n');
        time_a_day_seconds=(time_a_day.hour*3600+time_a_day.minute*60+time_a_day.second);///store time in second.
        time_morning_seconds=(time_a_day_seconds/2);       ///devive time by 2 for morning..
