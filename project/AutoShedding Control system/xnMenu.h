...
get=getch();
    ...
    if(mnu.ly+mnu.items+3<25)
        ClearBox(mnu.lx,mnu.ly,mnu.width+1,3+mnu.items,WINDOW_BG_TEXT);
    else
        ClearBox(mnu.lx,mnu.ly-1,mnu.width,3+mnu.items,WINDOW_BG_TEXT);
    return mnu.now_pos;
    }else if(get==13)
        return mnu.now_pos;
    else{
