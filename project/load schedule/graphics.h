/****
Title: Graphics library for a c program...
...
/***Structure to the Menu List********/

typedef struct{
                int lx,ly,width,items,now_pos;
                char *menu_text[25];
}Menu_list;
...
void LoadingAnimation(int lx,int ly,int width,int sleep_time,char *loading_title)
{
int i,j,text_color=3,sleep=0;
ShowConsoleCursor(FALSE);
gotoxy(lx,ly-2);    cprintf(loading_title,3);
    for(j=1;j<=2;j++){
        for(i=1;i<=width;i++){
                    gotoxy(lx,ly-1);    TextColor(11);   printf("%.2d %c",(i*100)/(width),(char)37);
                    gotoxy(lx+i-1,ly);    TextColor(text_color);  printf("%c",(char)219); ///219
                    gotoxy(lx+i-1,ly+1);    TextColor(text_color);  printf("%c",(char)223);   TextColor(7); ///223
                    Sleep(sleep);
        }
    sleep=sleep_time;
    text_color=15;
...
for(j=0;j<height;j++)
    ...
    gotoxy(0,2);    printf("%c",(char)199);
    gotoxy(0,22);    printf("%c",(char)199);
    gotoxy(79,2);    printf("%c",(char)182);
    gotoxy(79,22);    printf("%c",(char)182);
}
#endif
