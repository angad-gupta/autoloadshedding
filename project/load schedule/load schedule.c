#include<stdio.h>
#include"graphics.h"
#include"D:\CodeBlock\AutoSch\ls.h"
#include"D:\C\gr_h\date_converter.h"
...
int i,j,min_v=1,max_v=4,a;
    ...
    for(i=min_v;i<=max_v;i++){
        ...
        gotoxy(5+j*37,1+i*4-j*4*4+(i-min_v));    printf("Nigth:\t%.2d:%.2d:%.2d - %.2d:%.2d:%.2d",grp.day[i].night[0].hour,grp.day[i].night[0].minute,grp.day[i].night[0].second,grp.day[i].night[1].hour,grp.day[i].night[1].minute,grp.day[i].night[1].second);
...
Time t_gone,t_remaining,now,morning[2],night[2];
    ...
    night[1]=group[tb.now_pos].day[day].night[1];
        ...
        if(is_greater_time(morning[1],now) && is_greater_time(now,morning[0])){
            ...
            group[tb.now_pos].state=OFF;
        }
        else if(is_greater_time(now,night[0]) && is_greater_time(night[1],now)){
            t_remaining=subtract_time(night[1],now);
            t_gone=subtract_time(now,night[0]);
            group[tb.now_pos].state=ON;
        }else if(is_greater_time(now,morning[1]) && is_greater_time(night[0],now)){
            t_remaining=subtract_time(night[0],now);
            t_gone=subtract_time(now,morning[1]);
            group[tb.now_pos].state=OFF;
        }
        else if(is_greater_time(morning[0],now)){
            t_remaining=subtract_time(morning[0],now);
            night[1]=group[tb.now_pos].day[day-1].night[1];
            now.hour+=24;
            t_gone=subtract_time(now,night[1]);
            group[tb.now_pos].state=OFF;
        }
        ...
        printf(")");
            if(group[tb.now_pos].state==ON /*&& group[tb.now_pos].is_off==NO*/)
                    TextColor(124);
                else
                TextColor(112);
                gotoxy(48,18);
    ...
    switch(ch=getch()){
        ...
        case 'R':
            for(a=1;a<=7;a++)
                group[a]=load_group_ls_time("D:\\CodeBlock\\AutoSch\\ls-schedule.ls",a);
            draw_ls_schedule_tab_content(group[tb.now_pos]);
            handle_tab_control(tb);
            break;
        default:
...
gotoxy(0,24);
cprintf(" \x11\x10    ",115); cprintf("E",112);  cprintf("x",115);  cprintf("it.    \x1B",112); cprintf("ESC   R",115); cprintf("efresh     ",112);
gotoxy(0,0);
